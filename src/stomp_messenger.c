...
stomp_messenger_t *stomp_messenger_init() {
	...
	if (status != APR_SUCCESS) {
		...
		stomp_status_set(
			&ret->status, STOMP_FAILURE, "Unable to initialize the APR pool");
...
stomp_status_code_t stomp_set_endpoint(stomp_messenger_t *messenger, const char *uri) {
	...
	if (stat != APR_SUCCESS) {
		stomp_status_set(&messenger->status, STOMP_FAILURE, "Unable to parse the URI");
	...
	if (!messenger->uri.path) {
		stomp_status_set(
			&messenger->status, STOMP_FAILURE, "Invalid connection destination: null");
	...
	if (strlen(messenger->uri.path) == 0) {
		stomp_status_set(
			&messenger->status, STOMP_FAILURE, "Invalid connection destination: empty");
...
stomp_status_code_t stomp_connect(stomp_messenger_t *messenger,
	...
	if (stat != APR_SUCCESS) {
		...
		stomp_status_set(&messenger->status,
			STOMP_FAILURE,
	...
	if (header != NULL) {
		if (header->credentials != NULL) {
			apr_hash_set(conn_frame.headers,
				"login",
				APR_HASH_KEY_STRING,
	...
	apr_hash_set(conn_frame.headers, "accept-version", APR_HASH_KEY_STRING, "1.2");

	conn_frame.body = NULL;
	conn_frame.body_length = -1;

	stat = stomp_write(messenger->connection, &conn_frame, messenger->pool);
	...
	if (!stomp_io_can_read(messenger->connection)) {
		stomp_status_set(
			&messenger->status, STOMP_FAILURE, "Timed out while trying to read data");
...
static stomp_status_code_t stomp_process_receipt(stomp_messenger_t *messenger) {
	...
	if (strncmp(reply_frame->command, DISCONN_REPLY_STR, strlen(DISCONN_REPLY_STR)) !=
		0) {
		stomp_status_set(&messenger->status,
			STOMP_FAILURE,
			"Invalid disconnection reply: %s (%s)",
			reply_frame->command,
...
stomp_status_code_t stomp_subscribe(stomp_messenger_t *messenger,
	...
	if (messenger->options & STOMP_OPT_STRIP_ROOT) {
		apr_hash_set(
			frame.headers, "destination", APR_HASH_KEY_STRING, &messenger->uri.path[1]);
	} else {
		apr_hash_set(
			frame.headers, "destination", APR_HASH_KEY_STRING, messenger->uri.path);
	}
	...
	apr_hash_set(
		frame.headers, "id", APR_HASH_KEY_STRING, apr_itoa(messenger->pool, header->id));
...
stomp_status_code_t stomp_unsubscribe(stomp_messenger_t *messenger,
	...
	apr_hash_set(frame.headers, "destination", APR_HASH_KEY_STRING, messenger->uri.path);
	apr_hash_set(
		frame.headers, "id", APR_HASH_KEY_STRING, apr_itoa(messenger->pool, header->id));
...
stomp_status_code_t stomp_receive(stomp_messenger_t *messenger,
	...
	if (stat == APR_SUCCESS) {
		...
		messenger->exchange_properties->hash = apr_hash_overlay(
			messenger->pool, frame->headers, messenger->exchange_properties->hash);
