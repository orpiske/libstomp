...
stomp_engine_connect(stomp_connection **connection_ref,
	...
	if (connection == NULL) {
		stomp_status_set(
			&error, STOMP_FAILURE, "Not enough memory to create a connection");
		return APR_ENOMEM;
	}

	// Look up the remote address
	rc = apr_sockaddr_info_get(
		&connection->remote_sa, hostname, APR_UNSPEC, port, 0, pool);
	if (rc != APR_SUCCESS) {
		char msgbuf[80] = {0};
	...
	rc = apr_socket_create(
		&connection->socket, socket_family, SOCK_STREAM, APR_PROTO_TCP, pool);
	if (rc != APR_SUCCESS) {
		char msgbuf[80] = {0};
		...
		stomp_status_set(
			&error, STOMP_FAILURE, "unable to create socket: %s", hostname, msgbuf);
		return rc;
	}
	...
	if (rc != APR_SUCCESS && !APR_STATUS_IS_EAGAIN(rc)) {
		...
		stomp_status_set(&error, STOMP_FAILURE, "unable to connect: %s", msgbuf);
		return rc;
	}
	...
	if (rc != APR_SUCCESS) {
		...
		stomp_status_set(
			&error, STOMP_FAILURE, "unable to set socket options: %s", hostname, msgbuf);
		return rc;
	}
